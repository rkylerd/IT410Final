export const admin = {
    "username": "admin",
    "password": "admin",
    "role": "ADMIN",
    "email": "admin@admin.com"
};

export const items = [{
    id: 1,
    name: "Baseball Cap Black",
    price: 5,
    category: "baseball",
    description: "Black baseball cap",
    imageUrl: "black-baseball",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 2,
    name: "Baseball Cap Red",
    price: 5,
    category: "baseball",
    description: "Red baseball cap",
    imageUrl: "red-baseball",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 3,
    name: "Baseball Cap Blue",
    price: 5,
    category: "baseball",
    description: "Blue baseball cap",
    imageUrl: "blue-baseball",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 4,
    name: "Fedora Grey",
    price: 5,
    category: "fedora",
    description: "Grey Fedora",
    imageUrl: "grey-fedora",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 5,
    name: "Navy Blue Fedora",
    price: 5,
    category: "fedora",
    description: "Navy Blue Fedorea",
    imageUrl: "navyblue-fedora",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 6,
    name: "Cotton Beanie Black",
    price: 5,
    category: "beanie",
    description: "Cotton Beanie Black",
    imageUrl: "black-beanie",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 7,
    name: "Light Grey Cotton Beanie",
    price: 5,
    category: "beanie",
    description: "Light Grey Cotton Beanie",
    imageUrl: "lightgrey-beanie",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 8,
    name: "Green Cotton Beanie",
    price: 5,
    category: "beanie",
    description: "Green Cotton Beanie",
    imageUrl: "green-beanie",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 1,
    name: "Black Top Hat",
    price: 5,
    category: "top-hat",
    description: "Black Top Hat",
    imageUrl: "black-top-hat",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
},
{
    id: 1,
    name: "Grey Top Hat",
    price: 5,
    category: "top-hat",
    description: "Grey Top Hat",
    imageUrl: "grey-top-hat",
    stock: 2,
    sizes: ['small', 'medium', 'large'],
    promotionalFlag: false,
    promotionalPrice: 3
}];